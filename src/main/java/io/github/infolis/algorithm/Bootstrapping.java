...
public abstract class Bootstrapping extends BaseAlgorithm implements BootstrapLearner {
    ...
    List<TextualReference> getContextsForSeed(String seed) {
        ...
        // searching for terms is used to generate contexts for inducing new patterns, 
        // not for creating output.
        // Output contexts are those created by pattern-based search. Thus: do not post
        // textual references generated by SearchTermPosition, use temporary datastore client instead
        DataStoreClient tempClient = DataStoreClientFactory.create(DataStoreStrategy.TEMPORARY);
        Algorithm algo = execution.instantiateAlgorithm(getInputDataStoreClient(), tempClient, getInputFileResolver(), getOutputFileResolver());
        getOutputDataStoreClient().post(Execution.class, execution);
        algo.run();
