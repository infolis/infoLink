...
//			String regexB_flex_quoted = rightWords_quoted.get(0) + "\\s" + rightWords_quoted.get(1) + "\\s" + rightWords_quoted.get(2);
    ...
    //TODO: method used to store contexts for all reliable patterns instead of returning patterns - restore this behaviour!
    public static List<StudyContext> saveReliablePatternData(Set<StudyContext> contexts, double threshold, Set<String> processedMinimals, int size, Set<String> relInstances, Reliability r) throws IOException, ParseException {
        Set<InfolisPattern> patterns = new HashSet<>();
        Set<String> processedMinimals_iteration = new HashSet<>();
...
//			String regexB_flex_quoted = rightWords_quoted.get(0) + "\\s" + rightWords_quoted.get(1) + "\\s" + rightWords_quoted.get(2);
            ...
            for (InfolisPattern candidate : candidates) {
            	...
            	if (!nonStopwordPresent) log.debug("Pattern rejected - stopwords only");*/
            	//if (nonStopwordPresent & candidate.isReliable(size, relInstances, contexts, r).isEmpty()) {

            	//TODO: make sure patterns and contexts are posted etc
            	contextsOfReliablePatterns.addAll(candidate.getReliable(size, relInstances, contexts, r));
            	if (!contextsOfReliablePatterns.isEmpty()) {
            		//TODO: not necessary to add patterns
            		patterns.add(candidate);
            		processedMinimals_iteration.add(candidate.getMinimal());
            		log.debug("Pattern accepted");
            		//TODO (enhancement): separate number and character patterns: omit only less general patterns of the same type, do not limit generation of other type
